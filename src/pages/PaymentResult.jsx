...
export default function PaymentResult() {
  ...
  const btn = { display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(to left, #7b192c, #a82040)', color: '#f4be69', padding: '12px 26px', borderRadius: 50, fontWeight: 700, textDecoration: 'none', fontFamily: 'Tajawal, sans-serif' }
  ...
  if (state === 'loading') return (
    <div style={wrap}><div style={card}>
      ...
      <h2 style={{ color: '#3E1C00', fontFamily: 'Tajawal, sans-serif', margin: '18px 0 6px' }}>{isAr ? 'جارٍ تأكيد الدفع…' : 'Confirming payment…'}</h2>
  ...
  if (state === 'success') return (
    <div style={wrap}><div style={card}>
      ...
      <h2 style={{ color: '#3E1C00', fontFamily: 'Tajawal, sans-serif', margin: '18px 0 8px', fontSize: '1.5rem' }}>{isAr ? 'تمّ الدفع بنجاح!' : 'Payment Successful!'}</h2>
      ...
      {orderNumber && <p style={{ color: '#7b192c', fontWeight: 800, fontFamily: 'Tajawal, sans-serif', margin: '12px 0', fontSize: '1.05rem' }}>{isAr ? 'رقم الطلب: ' : 'Order #'}{orderNumber}</p>}
  ...
  return (
    <div style={wrap}><div style={card}>
      ...
      <h2 style={{ color: '#3E1C00', fontFamily: 'Tajawal, sans-serif', margin: '18px 0 8px', fontSize: '1.5rem' }}>{isAr ? 'لم يكتمل الدفع' : 'Payment Not Completed'}</h2>
      <p style={{ color: '#6B3A2A', fontSize: '0.92rem', lineHeight: 1.7 }}>
